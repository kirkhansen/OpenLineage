...
def test_get_event_log_records(mock_instance):
    ...
    run_updated_after = 10000000.0
    list(get_event_log_records(
        instance=mock_instance,
        event_types=[DagsterEventType.STEP_INPUT],
        run_updated_after=run_updated_after,
        record_filter_limit=record_filter_limit,
    ))

    mock_instance.get_event_records.assert_called_once_with(
        EventRecordsFilter(after_timestamp=run_updated_after, event_type=DagsterEventType.STEP_INPUT),
